...
func (param *UnownedBadgeParam) ParseBadge(values url.Values) error {
	param.Page = 1
	param.Limit = 10
	param.Sort = "desc"
	...
	if sortKey, ok := values["sort_key"]; ok && len(sortKey) > 0 {
		...
		if exist, _ := arrStr.InArray(sortKey[0], []string{"created_date"}); exist {
