...
func (param *UserFavouriteGameParam) ParseUserFavouriteGame(values url.Values) error {

	param.Page = 1
	param.Limit = 10
	param.Sort = "desc"
	param.Order = "total_play"
	param.Offset = 0
	...
	if order, ok := values["order"]; ok && len(order) > 0 {
		...
		if exist, _ := arrStr.InArray(order[0], []string{"total_play"}); exist {
			param.Order = order[0]
		}
	}
