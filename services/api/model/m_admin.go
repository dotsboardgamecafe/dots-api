...
func (c *Contract) GetAdminList(db *pgxpool.Pool, ctx context.Context, param request.AdminParam) ([]AdminEnt, request.AdminParam, error) {
	...
	if len(param.Keyword) > 0 {
		...
		orWhere = append(orWhere, fmt.Sprintf("a.name iLIKE $%d", len(paramQuery)))
		orWhere = append(orWhere, fmt.Sprintf("a.email iLIKE $%d", len(paramQuery)))
		where = append(where, "("+strings.Join(orWhere, " OR ")+")")
	}

	if len(param.Role) > 0 {
		var orWhere []string
		paramQuery = append(paramQuery, param.Role)
		orWhere = append(orWhere, fmt.Sprintf("r.name = $%d", len(paramQuery)))
	...
	if len(param.Status) > 0 {
		...
		orWhere = append(orWhere, fmt.Sprintf("a.status = $%d", len(paramQuery)))
		where = append(where, strings.Join(orWhere, " AND "))
	}

	// Handling Soft Delete
	where = append(where, "a.deleted_date IS NULL")
	...
	for rows.Next() {
		var data AdminEnt
		err = rows.Scan(&data.AdminCode, &data.Email, &data.Name, &data.UserName, &data.Status, &data.ImageURL, &data.PhoneNumber, &data.Role)
		if err != nil {
...
func (c *Contract) GetAdminIdsByCode(db *pgxpool.Pool, ctx context.Context, adminCodes []string) ([]int64, error) {
	var (
		err   error
		ids   []int64
		query = `SELECT id FROM admins WHERE admin_code = ANY($1)`
	)

	rows, err := db.Query(ctx, query, adminCodes)
	defer func() {
		rows.Close()
	}()
	...
	for rows.Next() {
		...
		err = rows.Scan(&id)
		if err != nil {
			return ids, c.errHandler("model.GetAdminIdsByCode", err, utils.ErrGettingAdminByCode)
		}
		ids = append(ids, id)
	}
...
func (c *Contract) GetAdminByCode(db *pgxpool.Pool, ctx context.Context, adminCode string) (AdminEnt, error) {
	var (
		err  error
		data AdminEnt
		sql  = `SELECT a.admin_code, a.email, a.name, a.username, a.password, a.status, r.name as role, a.phone_number, a.image_url
		FROM admins a left join roles r on r.id = a.role_id
		WHERE a.admin_code = $1`
	)

	err = db.QueryRow(ctx, sql, adminCode).Scan(&data.AdminCode, &data.Email, &data.Name, &data.UserName, &data.Password, &data.Status, &data.Role, &data.PhoneNumber, &data.ImageURL)
	if err != nil {
...
func (c *Contract) AddAdmin(db *pgxpool.Pool, ctx context.Context, adminCode, email, name, userName, password, status, role, phoneNumber, imageURL string) error {
	var (
		err    error
		roleId int
	)
...
func (c *Contract) UpdateAdminByCode(db *pgxpool.Pool, ctx context.Context, adminCode, email, name, userName, password, status, role, phoneNumber, imageURL string) error {
	var (
		err    error
		roleId int
		sql    = `
		UPDATE admins 
		SET email=$1,name=$2,username=$3,password=$4,status=$5,phone_number=$6,image_url=$7,updated_date=$8,role_id=$9
		WHERE admin_code = $10`
	)
	...
	_, err = db.Exec(ctx, sql, email, name, userName, password, status, phoneNumber, imageURL, time.Now().In(time.UTC), roleId, adminCode)
	if err != nil {
