...
func (c *Contract) GetAllClaimedInvoiceHistories(db *pgxpool.Pool, ctx context.Context, param request.UserClaimedHistoryParam) ([]UserRedeemHistoryDetailRes, request.UserClaimedHistoryParam, error) {
	var (
		err        error
		list       []UserRedeemHistoryDetailRes
		where      []string
		...
		SELECT
			...
			user_redeem_histories.updated_date AS updated_date
		FROM user_redeem_histories
			JOIN users u ON u.id = user_redeem_histories.user_id AND u.deleted_date IS NULL
	`
	)
	...
	// Populate Search
	{
		...
		err := db.QueryRow(ctx, newQcount, paramQuery...).Scan(&totalData)
		if err != nil {
			return list, param, c.errHandler("model.GetAllClaimedInvoiceHistories", err, utils.ErrCountingListUserRedeemHistory)
		}
		param.Count = totalData
	}
	...
	rows, err := db.Query(ctx, query, paramQuery...)
	if err != nil {
		return list, param, c.errHandler("model.GetAllClaimedInvoiceHistories", err, utils.ErrGettingListUserRedeemHistory)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			return list, param, c.errHandler("model.GetAllClaimedInvoiceHistories", err, utils.ErrScanningListUserRedeemHistory)
		}
		list = append(list, data)
	}
